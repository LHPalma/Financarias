...
| **RN-16** | **O pepper de desenvolvimento é exposto de propósito, para aprendizado.** A versão 1 está versionada em `appsettings.Development.json`, com comentário explícito. Um pepper só protege enquanto é segredo, então este não protege nada — existe para a aplicação subir em Development sem configuração extra e para estudar a técnica. É exceção consciente à RN-05 e à regra de nunca versionar segredo, decidida pelo autor. O que a mantém segura: o arquivo **só é carregado em Development**, então em Production o `ValidateOnStart` continua derrubando o boot até existir um pepper configurado fora do repositório. **Em produção, este valor deve ser tratado como comprometido**: não reutilizar, gerar um pepper novo em `PasswordHashing__Peppers__N` por variável de ambiente e rotacionar. Os funcionais sobem em `Testing`, que não lê esse arquivo, e recebem um pepper de teste por variável de ambiente no `TestBootstrap`. |
...
| 2 | `User` passa a exigir `PasswordHash` + migration que limpa e cria `password_hash` e `password_pepper_version` `NOT NULL` — decidir `OwnsOne` versus coluna separada | 1c |
