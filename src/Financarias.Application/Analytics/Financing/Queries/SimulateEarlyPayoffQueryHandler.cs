...
public class SimulateEarlyPayoffQueryHandler(
    SimulateEarlyPayoffMapper mapper
) : IQueryHandler<SimulateEarlyPayoffQuery, EarlyPayoffResult>
{
    public Task<EarlyPayoffResult> HandleAsync(
        SimulateEarlyPayoffQuery query,
        CancellationToken cancellationToken = default)
    {
        var schedule = FrenchAmortization.Schedule(query.Principal, query.MonthlyRate, query.Installments);

        return Task.FromResult(mapper.ToResult(schedule.PayoffAt(query.AtInstallment)));
    }
}
