...
///     <list type="number">
///         <item>
///             <description>Exponencial: <c>(1 + i)^(du/252)</c> — truncada a 14 casas (T-14)</description>
///         </item>
///         <item>
///             <description>Cotação: <c>100 / Exponencial</c> — truncada a 4 casas (T-4)</description>
///         </item>
///         <item>
///             <description>PU: <c>VNA × Cotação / 100</c> — truncada a 6 casas (T-6)</description>
///         </item>
///     </list>
...
public static class NtnbPricing
{
    ...
    public static decimal UnitPrice(decimal vna, AnnualYield yield, BusinessDayCount businessDays)
    {
        ...
        return Truncation.Truncate(vna * quotation / 100m, 6);
    }
    ...
    public static decimal Quotation(AnnualYield yield, BusinessDayCount businessDays)
    {
        ...
        var exponential = Truncation.Truncate(
            (decimal)Math.Pow((double)(1m + yield.Value), power), 14);

        return Truncation.Truncate(100m / exponential, 4);
    }
}